...
class Solution {
    public int findRadius(int[] houses, int[] heaters) {
        ...
        for (int house : houses) {
            ...
            if (nearestHeaterDistance > minRadius) {
                minRadius = nearestHeaterDistance;
            }
        }
        return minRadius;
    }
    ...
    private int findNearestHeaterDistance(int house, int[] heaters) {
        ...
        if (low > heaters.length - 1) {
            return house - heaters[high];
        }
        return Math.min(heaters[low] - house, house - heaters[high]);
    }
}